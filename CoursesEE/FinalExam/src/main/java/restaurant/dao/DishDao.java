...
public class DishDao implements MainMethodDao<Dish> {
    ...
    public List<Dish> selectAll() {
        LOGGER.info("Select menu category and all dishes!");
        Session session = sessionFactory.getCurrentSession();
...
//        dishDao.updateAllDishCategoryOnNOTCATEGORY(category);
    ...
    public void deleteCategoryDish(int id) {
        ...
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("DELETE Dish d SET  d.category.id = 1 WHERE d.id =:id");
        query.setParameter("id", id);
    }
...
//        return dishDao.selectIngredientsThisDish(idDish);
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    public List<DishIngredient> getAllIngredientsForDishes() {
        LOGGER.info("Select all ingredients for all dishes!");
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select di from DishIngredient di");
        return query.list();
    }
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    public void deleteIngredientsWithThisDish(int id) {
        LOGGER.info("Delete ingredient with dish!");
        Session session = sessionFactory.getCurrentSession();
        session.delete(findIngredient(id));
        session.flush();
    }
    @Transactional(propagation = Propagation.REQUIRED)
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    public DishIngredient findIngredient(int id) {
        LOGGER.info("Find ingredient with dish!");
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select d from DishIngredient d where d.id =:id");
        query.setParameter("id", id);
        if((DishIngredient) query.uniqueResult()==null){
            LOGGER.info("Not find ingredient with id: "+ id);
        } else{
            LOGGER.info("Find ingredient with id: "+ id);
        }
        return (DishIngredient) query.uniqueResult();
    }
    @Transactional(propagation = Propagation.REQUIRED)
