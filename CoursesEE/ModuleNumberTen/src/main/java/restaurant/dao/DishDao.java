...
//        dishDao.setDishCategory(id, numberCategory);
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    public List<Dish> selectAllNameDishCategoryMenu(int category) {
        LOGGER.info("Select all dishes one menu category id:" + category);
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select d.name from Dish d where d.category.id =:category");
        query.setParameter("category", category);
        return query.list();
    }
