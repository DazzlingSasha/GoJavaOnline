...
public class MenuDao implements MainMethodDao<Menu> {
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    public Menu findByOneName(String category) {
        LOGGER.info("Find menu by name: " + category);
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select m from Menu m where m.category =:category");
