...
public class WarehouseDao {
    ...
    public void createNewUser(Warehouse warehouse) {
    ...
    public List<Warehouse> findByNameWarehouse(String nameItem) {
        List<Warehouse> result = new ArrayList<>();
        String query = "SELECT * FROM  WAREHOUSE WHERE id_ingredient ILIKE " + "'%" + nameItem + "%'";
        try (Connection connection = dataSource.getConnection();
    ...
    @Transactional(propagation = Propagation.MANDATORY)
    public List<Warehouse> endsItemsInWarehouse() {
        List<Warehouse> result = new ArrayList<>();
        try (Connection connection = dataSource.getConnection();
             Statement statement = connection.createStatement()) {
