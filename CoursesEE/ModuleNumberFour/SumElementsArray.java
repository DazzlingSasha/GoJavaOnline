...
public class SumElementsArray implements SquareSum {
    List<Callable<Integer>> callables = new ArrayList<>();

    @Override
    public long getSquareSum(int[] values, int numberOfThreads) {
        int numberElementsInThread = (values.length % numberOfThreads == 0) ? values.length / numberOfThreads : (values.length / numberOfThreads) + 1;
        ...
        for (int i = 0; i < numberOfThreads; i++) {
            ...
            callables.add(new Callable<Integer>() {
                ...
                public Integer call() throws Exception {
                    System.out.println("Thread name: " + Thread.currentThread().getName() + " waiting" + phaser.getRoot());
